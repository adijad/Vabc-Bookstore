...
public class LineItemDaoJdbc implements LineItemDao {
    ...
    public long create(Connection connection, long bookId, long orderId, int quantity) {
        try (PreparedStatement statement = connection.prepareStatement(CREATE_LINE_ITEM_SQL)) {
            System.out.println("---- [LineItemDaoJdbc] BEFORE ----" + statement);
            statement.setLong(1, bookId);
            statement.setLong(2, orderId);
            statement.setInt(3, quantity);
            System.out.println("---- [LineItemDaoJdbc] AFTER ----" + statement);
            int affected = statement.executeUpdate();
            System.out.println("---- [LineItemDaoJdbc] STATEMENT EXEC ----" + affected);
            if (affected != 1) {
                System.out.println("Failed to insert an order line item, affected row count = " + affected);
...
//                throw new BookstoreUpdateDbException("Failed to retrieve lineItemId auto-generated key");
//            }
//            return lineItemId;
            return 1;
        } catch (SQLException e) {
            System.out.println("[LineItemDaoJdbc] Exception: " + e.getMessage());
